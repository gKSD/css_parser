...
#include "../src/select/font_face.h"
...
static void dump_media_query(css_media_query *media, char **ptr);
...
static const char *media_query_names[] = {
	"aural",
	"braille",
	"embossed",
	"handheld",
...
void dump_media_query(css_media_query *media, char **ptr)
{
	void *bytecode = media->bytecode;
	size_t length = (media->used * sizeof(css_code_t));
	uint32_t offset = 0;
...
#define ADVANCE(n) do { \
	...
	while (offset < length) {
		...
		if (flags & FLAG_NOT) {
			*ptr += sprintf(*ptr, " not ");
		} else if (flags & FLAG_ONLY) {
			*ptr += sprintf(*ptr, " only ");
		}
		...
		switch (op) {
			...
			case CSS_MEDIA_SPEECH:
			case CSS_MEDIA_TTY:
			case CSS_MEDIA_TV:
			case CSS_MEDIA_ALL:
				*ptr += sprintf(*ptr, " %s ", media_query_names[op]);
				break; // has no values
			case CSS_MEDIA_ASPECT_RATIO:
			case CSS_MEDIA_DEVICE_ASPECT_RATIO:
			case CSS_MEDIA_MAX_ASPECT_RATIO:
			case CSS_MEDIA_MAX_DEVICE_ASPECT_RATIO:
			case CSS_MEDIA_MIN_ASPECT_RATIO:
			case CSS_MEDIA_MIN_DEVICE_ASPECT_RATIO:
				// unsupported for now
				break;
			case CSS_MEDIA_COLOR:
			case CSS_MEDIA_COLOR_INDEX:
			case CSS_MEDIA_MONOCHROME:
			case CSS_MEDIA_GRID: {
				*ptr += sprintf(*ptr, "(%s ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_COLOR_SET:
						*((*ptr)++) = ':';
			...
			case CSS_MEDIA_MAX_COLOR:
			case CSS_MEDIA_MAX_COLOR_INDEX:
			case CSS_MEDIA_MIN_COLOR:
			case CSS_MEDIA_MIN_COLOR_INDEX:
			case CSS_MEDIA_MAX_MONOCHROME:
			case CSS_MEDIA_MIN_MONOCHROME: {
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				css_fixed val = *((css_fixed *) bytecode);
				ADVANCE(sizeof(val));
				dump_number(val, ptr);
			...
			case CSS_MEDIA_DEVICE_HEIGHT:
			case CSS_MEDIA_DEVICE_WIDTH:
			case CSS_MEDIA_HEIGHT:
			case CSS_MEDIA_MAX_DEVICE_HEIGHT:
			case CSS_MEDIA_MAX_DEVICE_WIDTH:
			case CSS_MEDIA_MAX_HEIGHT:
			case CSS_MEDIA_MIN_DEVICE_WIDTH:
			case CSS_MEDIA_MIN_DEVICE_HEIGHT:
			case CSS_MEDIA_MIN_HEIGHT:
			case CSS_MEDIA_MAX_WIDTH:
			case CSS_MEDIA_MIN_WIDTH:
			case CSS_MEDIA_WIDTH: {
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				css_fixed val = *((css_fixed *) bytecode);
				ADVANCE(sizeof(val));
			...
			case CSS_MEDIA_MAX_RESOLUTION:
			case CSS_MEDIA_MIN_RESOLUTION:
			case CSS_MEDIA_RESOLUTION:{
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				css_fixed val = *((css_fixed *) bytecode);
				ADVANCE(sizeof(val));
				uint32_t unit = *((uint32_t *) bytecode);
			...
			case CSS_MEDIA_ORIENTATION:
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_ORIENTATION_LANDSCAPE:
						*ptr += sprintf(*ptr, "landscape");
						break;
					case MEDIA_FEATURE_ORIENTATION_PORTRAIT:
						*ptr += sprintf(*ptr, "portrait");
						break;
				}
				*((*ptr)++) = ')';
				break;
			case CSS_MEDIA_OVERFLOW_BLOCK:
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_OVERFLOW_BLOCK_NONE:
						*ptr += sprintf(*ptr, "none");
						break;
					case MEDIA_FEATURE_OVERFLOW_BLOCK_SCROLL:
						*ptr += sprintf(*ptr, "scroll");
						break;
					case MEDIA_FEATURE_OVERFLOW_BLOCK_OPTIONAL_PAGED:
						*ptr += sprintf(*ptr, "optional-paged");
						break;
					case MEDIA_FEATURE_OVERFLOW_BLOCK_PAGED:
						*ptr += sprintf(*ptr, "paged");
						break;
				}
				*((*ptr)++) = ')';
				break;
			case CSS_MEDIA_OVERFLOW_INLINE:
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_OVERFLOW_INLINE_NONE:
						*ptr += sprintf(*ptr, "none");
						break;
					case MEDIA_FEATURE_OVERFLOW_INLINE_SCROLL:
						*ptr += sprintf(*ptr, "scroll");
						break;
				}
				*((*ptr)++) = ')';
			...
			case CSS_MEDIA_SCAN:
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_SCAN_PROGRESSIVE:
						*ptr += sprintf(*ptr, "progressive");
						break;
					case MEDIA_FEATURE_SCAN_INTERLACE:
						*ptr += sprintf(*ptr, "interlace");
						break;
				}
				*((*ptr)++) = ')';
				break;
			case CSS_MEDIA_UPDATE_FREQUENCY:
				*ptr += sprintf(*ptr, "(%s: ", media_query_names[op]);
				switch (value) {
					case MEDIA_FEATURE_UPDATE_FREQUENCY_NONE:
						*ptr += sprintf(*ptr, "none");
						break;
					case MEDIA_FEATURE_UPDATE_FREQUENCY_SLOW:
						*ptr += sprintf(*ptr, "slow");
						break;
					case MEDIA_FEATURE_UPDATE_FREQUENCY_FAST:
		...
		if (flags & FLAG_DELIMITER_COMMA) {
			*((*ptr)++) = ',';
		} else if (flags & FLAG_DELIMITER_AND) {
			*ptr += sprintf(*ptr, " and ");
		}
	}
...
void dump_number(css_fixed val, char **ptr)
{
...
void dump_unit(css_fixed val, uint32_t unit, char **ptr)
{
	dump_number(val, ptr);
	...
	case UNIT_DPI:
		*ptr += sprintf(*ptr, "dpi");
		break;
	case UNIT_DPCM:
		*ptr += sprintf(*ptr, "dpcm");
		break;
	case UNIT_DPPX:
		*ptr += sprintf(*ptr, "dppx");
		break;
	}
}
		...
		if (isInherit(opv)) {
			*ptr += sprintf(*ptr, "inherit");
				...
				switch (value) {
					...
					case BACKGROUND_SIZE_CONTAIN:
						*ptr += sprintf(*ptr, "contain");
						break;
					default: // [auto | length | percentage] {1,2}
						value = *((uint32_t *) bytecode);
						ADVANCE(sizeof(value));

						while (value != BACKGROUND_SIZE_END) {
							if (value == BACKGROUND_SIZE_AUTO) {
								*ptr += sprintf(*ptr, "auto");
							}
							else if (value == BACKGROUND_SIZE_VALUE) {
								uint32_t unit;
								css_fixed val = *((css_fixed *) bytecode);
								ADVANCE(sizeof(val));
								unit = *((uint32_t *) bytecode);
			...
			case CSS_PROP_BORDER_BOTTOM_RIGHT_RADIUS:
				...
				while (value != BORDER_RADIUS_END) {

					if (value == BORDER_RADIUS_DIMENSION_VALUE) {
						uint32_t unit;
						css_fixed val = *((css_fixed *) bytecode);
						ADVANCE(sizeof(val));
						unit = *((uint32_t *) bytecode);
						ADVANCE(sizeof(unit));
						dump_unit(val, unit, ptr);
					} else if (value == BORDER_RADIUS_NUMBER_VALUE) {
						css_fixed val = *((css_fixed *) bytecode);
						ADVANCE(sizeof(val));
						dump_number(val, ptr);
					}
