...
#include "../../../libparserutils/include/parserutils/utils/stack.h"
...
static css_error parseMediaFeatureDimensionValue(css_language *c,
		const parserutils_vector *vector, int *ctx,
		css_fixed *length, uint32_t *unit, bool *has_value);
static css_error parseMediaFeatureIntegerValue(const parserutils_vector *vector,
		int *ctx, css_fixed *num,
		bool *has_value, bool is_bool_value);
...
css_error handleStartAtRule(css_language *c, const parserutils_vector *vector)
{
	 ...
	 * there is one */
	if (atkeyword == NULL || atkeyword->type != CSS_TOKEN_ATKEYWORD)
		return CSS_INVALID;
	...
	if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[CHARSET], 
		...
		if (c->state <= IMPORT_PERMITTED) {
			...
			/* allocate media */
			error = css__stylesheet_media_query_create(&media);
			if (error != CSS_OK)
				return error;
			...
			if (error != CSS_OK) {
				css__stylesheet_media_query_destroy(media);
				return error;
			}
...
static css_error processMediaFeatureOptionalBooleanValue(const parserutils_vector *vector,
		int *ctx, css_media_type type, css_code_t *result,
		int result_len, int *result_pos, bool *has_value)
{
	if (result_len < 2)
		return CSS_BADPARM;

	appendOpcode(&result[0], type);
	css_fixed num = 0;
	css_error error = parseMediaFeatureIntegerValue(vector, ctx, &num, has_value, true);
	if (error != CSS_OK) {
		return error;
	}
...
static css_error processMediaFeatureOptionalIntegerValue(const parserutils_vector *vector,
		int *ctx, css_media_type type, css_code_t *result,
		int result_len, int *result_pos, bool *has_value)
{
	if (result_len < 2)
	...
	appendOpcode(&result[0], type);
	css_fixed num = 0;
	css_error error = parseMediaFeatureIntegerValue(vector, ctx, &num, has_value, false);
	if (error != CSS_OK) {
		return error;
	}
...
static css_error processMediaFeatureIntegerValue(const parserutils_vector *vector,
		int *ctx, css_media_type type,
		css_code_t *result, int result_len, int *result_pos)
{
	if (result_len < 2)
		return CSS_BADPARM;
...
static css_error processMediaFeatureDimensionValue(css_language *c,
		const parserutils_vector *vector, int *ctx, css_media_type type,
		css_code_t *result, int result_len, int *result_pos)
{
	if (result_len < 3)
		return CSS_BADPARM;
	...
	uint32_t unit = 0;
	bool has_value;
	css_error error = parseMediaFeatureDimensionValue(c, vector, ctx, &length, &unit, &has_value);
	if (error != CSS_OK) {
		return error;
	}
	if (!has_value)
		...
		css_style *media)
{
	...
	while (token != NULL) {
		...
		if (tokenIsString(token, c->strings[NOT])) {
			appendFlags(&bytecodes[0], FLAG_NOT);
			read_next_token = true;
		} else if (tokenIsString(token, c->strings[ONLY])) {
			appendFlags(&bytecodes[0], FLAG_ONLY);
			read_next_token = true;
		}
		...
		if (token->type == CSS_TOKEN_IDENT) {
			if (tokenIsString(token, c->strings[AURAL])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_AURAL);
			} else if (tokenIsString(token, c->strings[BRAILLE])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_BRAILLE);
			} else if (tokenIsString(token, c->strings[EMBOSSED])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_EMBOSSED);
			} else if (tokenIsString(token, c->strings[HANDHELD])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_HANDHELD);
			} else if (tokenIsString(token, c->strings[PRINT])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_PRINT);
			} else if (tokenIsString(token, c->strings[PROJECTION])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_PROJECTION);
			} else if (tokenIsString(token, c->strings[SCREEN])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_SCREEN);
			} else if (tokenIsString(token, c->strings[SPEECH])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_SPEECH);
			} else if (tokenIsString(token, c->strings[TTY])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_TTY);
			} else if (tokenIsString(token, c->strings[TV])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_TV);
			} else if (tokenIsString(token, c->strings[ALL])) {
				appendOpcode(&bytecodes[0], CSS_MEDIA_ALL);
			} else
				return CSS_INVALID;
				...
				if (tokenIsString(token, c->strings[MEDIA_FEATURE_ASPECT_RATIO])) {
					// TODO: unsupported
					return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_COLOR])) {
					bool has_value;
					error = processMediaFeatureOptionalIntegerValue(vector, ctx,
							CSS_MEDIA_COLOR, bytecodes, 3,
							&bytecode_pos, &has_value);
					if (error != CSS_OK)
						return error;
					appendValue(&bytecodes[0], has_value ? MEDIA_FEATURE_COLOR_SET :
							MEDIA_FEATURE_COLOR_NO_VALUE);
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_COLOR_INDEX])) {
					bool has_value;
					error = processMediaFeatureOptionalIntegerValue(vector, ctx,
							CSS_MEDIA_COLOR_INDEX, bytecodes, 3,
							&bytecode_pos, &has_value);
					if (error != CSS_OK)
						return error;
					appendValue(&bytecodes[0], has_value ? MEDIA_FEATURE_COLOR_INDEX_SET :
							MEDIA_FEATURE_COLOR_INDEX_NO_VALUE);
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_DEVICE_ASPECT_RATIO])) {
					return CSS_INVALID; //TODO
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_DEVICE_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_DEVICE_HEIGHT, bytecodes,
							3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_DEVICE_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_DEVICE_WIDTH, bytecodes,
							3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_GRID])) {
					bool has_value;
					error = processMediaFeatureOptionalBooleanValue(vector, ctx,
							CSS_MEDIA_GRID, bytecodes, 3, &bytecode_pos,
							&has_value);
					if (error != CSS_OK)
						return error;
					appendValue(&bytecodes[0], has_value ? MEDIA_FEATURE_GRID_SET :
							MEDIA_FEATURE_GRID_NO_VALUE);
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_HEIGHT, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_ASPECT_RATIO])) {
					return CSS_INVALID; //TODO
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_COLOR])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MAX_COLOR, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_COLOR_INDEX])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MAX_COLOR_INDEX, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_DEVICE_ASPECT_RATIO])) {
					return CSS_INVALID; //TODO
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_DEVICE_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MAX_DEVICE_HEIGHT, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_DEVICE_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MAX_DEVICE_WIDTH, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MAX_HEIGHT, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_MONOCHROME])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MAX_MONOCHROME, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_RESOLUTION])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MAX_RESOLUTION, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MAX_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MAX_WIDTH, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_ASPECT_RATIO])) {
					return CSS_INVALID; //TODO
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_COLOR])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MIN_COLOR, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_COLOR_INDEX])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MIN_COLOR_INDEX, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_DEVICE_ASPECT_RATIO])) {
					return CSS_INVALID; //TODO
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_DEVICE_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MIN_DEVICE_HEIGHT, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_DEVICE_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MIN_DEVICE_WIDTH, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_HEIGHT])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MIN_HEIGHT, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_MONOCHROME])) {
					error = processMediaFeatureIntegerValue(vector, ctx,
							CSS_MEDIA_MIN_MONOCHROME, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_RESOLUTION])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MIN_RESOLUTION, bytecodes, 3,
							&bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MIN_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_MIN_WIDTH, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_MONOCHROME])) {
					bool has_value;
					error = processMediaFeatureOptionalIntegerValue(vector, ctx,
							CSS_MEDIA_MONOCHROME, bytecodes, 3,
							&bytecode_pos, &has_value);
					if (error != CSS_OK)
						return error;
					appendValue(&bytecodes[0], has_value ? MEDIA_FEATURE_MONOCHROME_SET :
							MEDIA_FEATURE_MONOCHROME_NO_VALUE);
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_ORIENTATION])) {
					appendOpcode(&bytecodes[0], CSS_MEDIA_ORIENTATION);
					consumeWhitespace(vector, ctx);
					token = parserutils_vector_iterate(vector, ctx);
					...
					if (tokenIsString(token, c->strings[MEDIA_FEATURE_LANDSCAPE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_ORIENTATION_LANDSCAPE);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_PORTRAIT])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_ORIENTATION_PORTRAIT);
					} else {
						return CSS_INVALID;
					}
					consumeWhitespace(vector, ctx);
					...
					if (!tokenIsChar(token, ')'))
						return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_OVERFLOW_BLOCK])) {
					appendOpcode(&bytecodes[0], CSS_MEDIA_OVERFLOW_BLOCK);
					consumeWhitespace(vector, ctx);
					token = parserutils_vector_iterate(vector, ctx);
					if (!tokenIsChar(token, ':'))
						return CSS_INVALID;
					...
					if (tokenIsString(token, c->strings[MEDIA_FEATURE_NONE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_BLOCK_NONE);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_SCROLL])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_BLOCK_SCROLL);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_OPTIONAL_PAGED])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_BLOCK_OPTIONAL_PAGED);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_PAGED])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_BLOCK_PAGED);
					} else {
						return CSS_INVALID;
					}
					...
					if (!tokenIsChar(token, ')'))
						return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_OVERFLOW_INLINE])) {
					appendOpcode(&bytecodes[0], CSS_MEDIA_OVERFLOW_INLINE);
					consumeWhitespace(vector, ctx);
					token = parserutils_vector_iterate(vector, ctx);
					if (!tokenIsChar(token, ':'))
						return CSS_INVALID;
					...
					if (tokenIsString(token, c->strings[MEDIA_FEATURE_NONE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_INLINE_NONE);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_SCROLL])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_OVERFLOW_INLINE_SCROLL);
					} else {
						return CSS_INVALID;
					}
					...
					if (!tokenIsChar(token, ')'))
						return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_RESOLUTION])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_RESOLUTION, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_SCAN])) {
					appendOpcode(&bytecodes[0], CSS_MEDIA_SCAN);
					consumeWhitespace(vector, ctx);
					token = parserutils_vector_iterate(vector, ctx);
					if (!tokenIsChar(token, ':'))
						return CSS_INVALID;
					...
					if (tokenIsString(token, c->strings[MEDIA_FEATURE_PROGRESSIVE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_SCAN_PROGRESSIVE);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_INTERLACE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_SCAN_INTERLACE);
					} else {
						return CSS_INVALID;
					}
					...
					if (!tokenIsChar(token, ')'))
						return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_UPDATE_FREQUENCY])) {
					appendOpcode(&bytecodes[0], CSS_MEDIA_UPDATE_FREQUENCY);
					consumeWhitespace(vector, ctx);
					token = parserutils_vector_iterate(vector, ctx);
					if (!tokenIsChar(token, ':'))
						return CSS_INVALID;
					...
					if (tokenIsString(token, c->strings[MEDIA_FEATURE_NONE])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_UPDATE_FREQUENCY_NONE);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_SLOW])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_UPDATE_FREQUENCY_SLOW);
					} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_FAST])) {
						appendValue(&bytecodes[bytecode_pos], MEDIA_FEATURE_UPDATE_FREQUENCY_FAST);
					} else {
						return CSS_INVALID;
					}
					consumeWhitespace(vector, ctx);
					...
					if (!tokenIsChar(token, ')'))
						return CSS_INVALID;
				} else if (tokenIsString(token, c->strings[MEDIA_FEATURE_WIDTH])) {
					error = processMediaFeatureDimensionValue(c, vector, ctx,
							CSS_MEDIA_WIDTH, bytecodes, 3, &bytecode_pos);
					if (error != CSS_OK)
						return error;
				} else
					return CSS_INVALID;
		...
		// parsing media features delimiter
		consumeWhitespace(vector, ctx);

		uint8_t tmp_flags = 0;
		token = parserutils_vector_iterate(vector, ctx);
		if (token != NULL) {
			if (tokenIsString(token, c->strings[AND])) {
				tmp_flags |= FLAG_DELIMITER_AND;
			} else if (tokenIsChar(token, ',')) {
				tmp_flags |= FLAG_DELIMITER_COMMA;
			} else
				return CSS_INVALID;
		...
		appendFlags(&bytecodes[0], tmp_flags);

		error = css__stylesheet_style_append(media, bytecodes[0]);
		if (error != CSS_OK) {
			return error;
		}

		for (int i = 1; i <= bytecode_pos; ++i) {
			error = css__stylesheet_style_append(media, bytecodes[i]);
			if (error != CSS_OK) {
				return error;
			}
		}
...
css_error parseMediaFeatureDimensionValue(css_language *c,
		const parserutils_vector *vector, int *ctx,
		css_fixed *length, uint32_t *unit, bool *has_value)
{
	consumeWhitespace(vector, ctx);
	...
	css_error error = CSS_OK;
	const css_token *token = parserutils_vector_iterate(vector, ctx);
...
css_error parseMediaFeatureIntegerValue( const parserutils_vector *vector,
		int *ctx, css_fixed *num,
		bool *has_value, bool is_bool_value)
{
	consumeWhitespace(vector, ctx);
			...
			*has_value = false;
			...
			if (token != NULL && token->type == CSS_TOKEN_NUMBER) {
				...
				*num = css__number_from_lwc_string(token->idata, true, &consumed);
				...
				if (is_bool_value && *num != 0 && *num != 1024) // 1024 -> 1, 0 -> 0
					return CSS_INVALID;
