...
static css_error _style_create_impl(css_stylesheet *sheet, css_style **style);
...
css_error css_stylesheet_next_pending_import(css_stylesheet *parent,
		lwc_string **url, css_media_query **media)
{
	const css_rule *r;
...
css_error css__stylesheet_style_create(css_stylesheet *sheet, css_style **style)
{
	...
	return _style_create_impl(sheet, style);
}

/* allowing sheet == NULL (needed for css_media_rule) */
css_error _style_create_impl(css_stylesheet *sheet, css_style **style)
{
	css_style *s;
...
/**
 * Destroy a media query list
 *
 * \param media         The media qeury to destroy
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error css__stylesheet_media_query_destroy(css_media_query *media)
{
	return css__stylesheet_style_destroy(media);
}
 ...
 * \param sheet         The stylesheet context
 * \param len   The required length of the style
 * \param media         Pointer to location to receive style
 * \return CSS_OK on success,
 *        CSS_BADPARM on bad parameters,
 *        CSS_NOMEM on memory exhaustion
 */
css_error css__stylesheet_media_query_create(css_media_query **media)
{
	return _style_create_impl(NULL, media);
}
...
css_error css__stylesheet_rule_destroy(css_stylesheet *sheet, css_rule *rule)
{
	...
	if (rule->parent != NULL || rule->next != NULL ||
			rule->prev != NULL)
		return CSS_INVALID;
	...
	case CSS_RULE_IMPORT:
	{
		...
		if (import->media != NULL)
			css__stylesheet_media_query_destroy(import->media);
	...
	case CSS_RULE_MEDIA:
	{
		...
		if (media->media != NULL)
			css__stylesheet_media_query_destroy(media->media);
...
css_error css__stylesheet_rule_set_media(css_stylesheet *sheet,
		css_rule *rule, css_media_query *media)
{
...
css_error css_stylesheet_media_to_string (const css_media_query *media, lwc_string **result)
{
	// TODO: parse bytecode
	UNUSED(media);
	UNUSED(result);
...
bool css__stylesheet_media_query_applies_to_media(uint64_t applicable_media, css_media_query *media)
{
	// TODO: check bytecode array in media
	UNUSED(applicable_media);
	UNUSED(media);
	return true;
}
