...
#include "../../include/libcss/select.h"
	...
	do {
		...
		if (rule != NULL && rule->type == CSS_RULE_IMPORT) {
			...
			if (import->sheet != NULL &&
					css__stylesheet_media_query_applies_to_media(state->media, import->media)) {
...
static inline bool _rule_applies_to_media(const css_rule *rule, uint64_t media)
{
	...
	while (ancestor != NULL) {
		...
		if (ancestor->type == CSS_RULE_MEDIA &&
				!css__stylesheet_media_query_applies_to_media(media, m->media)) {
				//(m->media & media) == 0) {
			applies = false;
			break;
		}
